...
class TestRecordsNew(unittest2.TestCase):
    ...
    def test_files_modified_earlier_than_days(self):
        ...
        concord_cash_file = join(get_current_path(), 'samples', 'Reconciliation', 'Concord', 'Cash Stt _ 20072017.xlsx')
        days = (datetime.now() - datetime(2017,7,21)).days     # num of days between now and 2017-7-21
    ...
    def test_db_record_exists(self):
        ...
        old_concord_file = join(get_current_path(), 'samples', 'Reconciliation', 'Concord', 'Cash Stt _ 20072017.xlsx')

        files = [concord_holding_file, concord_cash_file, ffx_holding_file, 
                    ffx_cash_file, sample_txt_file, old_concord_file]
